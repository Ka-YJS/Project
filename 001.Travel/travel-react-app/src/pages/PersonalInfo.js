...
const PersonalInfo = () => {
  ...
  const handleProfileImageChange = async (e) => {
    ...
    if (file) {
      ...
      try {
        // 백엔드에 프로필 사진을 업로드
        const response = await axios.patch(`http://${config.IP_ADD}/travel/userProfileImageEdit/${user.id}`, formData, {
          headers: {
  ...
  const handleProfileImageDelete = async () => {
    ...
    try {      
      ...
      if(user.userProfileImage !== null){        
        ...
        const response = await axios.patch(`http://${config.IP_ADD}/travel/userProfileImageDelete/${user.id}`,null, {
          headers: {
            'Authorization': `Bearer ${user.token}`
          },
        });
      ...
      <div className="wrapper">
        <div className="profile_wrapper ">
          <img
            ...
            src={user.userProfileImage?`http://${config.IP_ADD}${user.userProfileImage}`: defaultImage}
            alt="profile"
          />
